...
async def button(bot, update: CallbackQuery):
    ...
    if cb_data.startswith("gUPcancel"):
        ...
        if (int(update.from_user.id) == int(from_usr)) or g:
            ...
            await bot.answer_callback_query(
                ...
                text="U Naughty 😁, U R Not Allowed",
                show_alert=True,
                cache_time=0,
            )
        return
    ...
    if cb_data.startswith("cancel"):
            ...
            if len(cb_data) > 1:
                ...
                try:
                    ...
                    await i_m_s_e_g.edit_text(
                        f"Leech Cancelled by <a> href='tg://user?id={update.from_user.id}'>{update.from_user.first_name} So Tata GdBye Khatam</a>"
                    )
        ...
        if (update.from_user.id in AUTH_CHANNEL) or g:
            ...
            await update.message.edit_text("U Naughty 😁, U R Not Allowed 🤭")
